...
internal class Info(
    ...
    : IInfo
{
    private readonly string _description = string.Format(assembly?.GetCustomAttribute<AssemblyDescriptionAttribute>()?.Description ?? "C# {0} script runner", languageVersion.ToDisplayString());
    private readonly string _version = assembly?.GetCustomAttribute<AssemblyInformationalVersionAttribute>()?.InformationalVersion ?? string.Empty;

    public void ShowHeader()
    {
        stdOut.WriteLine(new Text(_description, Color.Header));
        if (settings.InteractionMode != InteractionMode.Interactive)
        {
            return;
        }

        stdOut.WriteLine(new Text("Ctrl-C - Exit the REPL.", Color.Highlighted));
        stdOut.WriteLine(new Text("#help  - Display help on available commands and key bindings.", Color.Highlighted));
    }
    ...
    public void ShowReplHelp()
    {
        ...
        var settingLines =
            from setting in settingDescriptions
    ...
    public void ShowHelp() =>
        stdOut.WriteLine(
            new []
            {
                [
                    Text.NewLine,
                    new Text("Usage:", Color.Header),
                    new Text(" dotnet csi [options] [--] [script] [script arguments]"), Text.NewLine,
                    Text.NewLine,
                    new Text("Executes a script if specified, otherwise launches an interactive REPL (Read Eval Print Loop)."),
                    Text.NewLine,
                    Text.NewLine
                ],
                Option(
                ...
                [
                    new Text("Notes:", Color.Header),
                    Text.NewLine,
                    Text.Tab,
                    new Text("- "),
                    new Text("*1", Color.Highlighted),
                    new Text(" script properties are accessible a script via the global dictionary "),
                    new Text("Props[\"name\"]", Color.Details),
                    new Text(" by a property name"),
                    Text.NewLine,
                    Text.Tab,
                    new Text("- "),
                    new Text("using HostApi;", Color.Details),
    ...
    public void ShowVersion() => stdOut.WriteLine(new Text(_version));

    public void ShowFooter() => tracePresenter.Show(traceSources);
